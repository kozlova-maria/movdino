...
// PAINT красит клетку под динозавром
...
int main(int argc, char *argv[]) {
    ...
    // Выделение памяти для матрицы (сначала с минимальными размерами)
    char ***matrix = NULL;
    int size_found = 0;
    int start_found = 0;
    int line_number = 0;
    ...
    while (fgets(line, sizeof(line), input_file) != NULL) {
        ...
        if (col_str == 1 && wordcount > 0 && 
            strcmp(str[0], "SIZE") != 0 ) {
            fprintf(output_file, "Ошибка: команды должны начинаться с SIZE\n");
            printf("Ошибка: команды должны начинаться с SIZE\n");
            fclose(input_file);
            fclose(output_file);
            return 1;
        }
        ...
        // Обработка SIZE
        if (col_str == 1 && wordcount == 3 && strcmp(str[0], "SIZE") == 0 && 
            ...
            // Выделение памяти для матрицы после получения SIZE
         ...
         if (col_str == 2 && wordcount > 0 && 
            strcmp(str[0], "START") != 0 ) {
            fprintf(output_file, "Ошибка: команды должны начинаться с START\n");
            printf("Ошибка: команды должны начинаться с START\n");
            fclose(input_file);
            fclose(output_file);
            return 1;
        }
        ...
        // Обработка START
        if (col_str == 2 && wordcount == 3 && strcmp(str[0], "START") == 0 && 
            isdigit(str[1][0]) && isdigit(str[2][0])) {
            ...
            if (start_x >= 0 && start_x < width && start_y >= 0 && start_y < height) {
                ...
                printf("Ошибка: стартовые координаты вне диапазона поля\n");
                fclose(input_file);
                fclose(output_file);
                return 1;
            }
        ...
        // Проверка повторных SIZE и START
        if (col_str > 1 && wordcount == 3 && strcmp(str[0], "SIZE") == 0) {
            fprintf(output_file, "Ошибка: повторная команда SIZE\n");
            printf("Ошибка: повторная команда SIZE\n");
            fclose(input_file);
            fclose(output_file);
            return 1;
        }
        ...
        if (col_str > 2 && wordcount == 3 && strcmp(str[0], "START") == 0) {
            fprintf(output_file, "Ошибка: повторная команда START\n");
            printf("Ошибка: повторная команда START\n");
            fclose(input_file);
            fclose(output_file);
            return 1;
        }
        if (col_str < 2 ) {
            fprintf(output_file, "Ошибка: нет size или start\n");
            printf("Ошибка: нет size или start\n");
            fclose(input_file);
            fclose(output_file);
            return 1; }
        ...
        fprintf(output_file, "Выполнение команды: %s\n", line);
        ...
        if (command_valid == 0) {
            fprintf(output_file, "Ошибка в строке  %d\n", line_number);
